...
// never as a misleading empty board (empty success keeps the honest sync copy).
export default function Error({ reset }: { reset: () => void }) {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#F4F9FF] via-white to-[#F0F7FF] flex items-center justify-center px-5 py-16">
      <div
        role="alert"
        className="max-w-lg w-full rounded-3xl border border-dashed border-rose-200 bg-white p-10 text-center space-y-3 shadow-xs"
      >
        <p className="text-base font-bold text-rose-600">Không thể tải Bảng Vàng Danh Dự.</p>
